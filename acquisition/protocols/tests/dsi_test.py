...
def test_eeg_packet():
    ...
    params = dict(type='EEG_DATA', payload_length=19, number=13,
                  timestamp=559315520.0, data_counter=0, ADC_status=b"123456",
                  sensor_data=[484.799987793, -502.200012207])
    result = dsi.packet.build(params)
    assert result == expected
    ...
    for k in params.keys():
        if k != 'sensor_data':
            assert c[k] == params[k]
    assert ("%.9f" % c['sensor_data'][0]) == '484.799987793'
    assert ("%.9f" % c['sensor_data'][1]) == '-502.200012207'
