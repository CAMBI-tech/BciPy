...
def symbol_positions(layout: Layout, rows: int,
    ...
    Parameters
    ----------
        layout - layout in which the grid should be placed
    ...
    # compute the spacing (in layout units) from the layout width and height
    horizontal_spacing = layout.width / (rows + 1)
    vertical_spacing = layout.height / (columns + 1)
    ...
    start_pos_left = layout.left_of(center_x, spaces_left_of_center * spacing)
    if columns % 2 == 0 and columns > 1:
        # even number of columns; adjust start_pos so that center_x is between
        # the middle two items.
        start_pos_left = layout.right_of(start_pos_left, half_space)

    spaces_above_center = int(rows / 2)
    start_pos_top = layout.above(center_y, spaces_above_center * spacing)
    if rows % 2 == 0 and rows > 1:
        start_pos_top = layout.below(start_pos_top, half_space)

    # adjust the beginning x,y values so adding a space results in the first
    # position.
    x_coord = layout.left_of(start_pos_left, spacing)
    y_coord = layout.above(start_pos_top, spacing)

    positions = []
    for _row in range(rows):
        y_coord = layout.below(y_coord, spacing)
        x_coord = layout.left_of(start_pos_left, spacing)
        for _col in range(columns):
            x_coord = layout.right_of(x_coord, spacing)
