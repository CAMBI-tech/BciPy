...
class PromptLanguageModel(LanguageModel):
    """Character language model based on prompt engineering a pre-trained and task-tuned causal model."""
    ...
    def _encode(self, text: str) -> List[int]:
        tokens = self.tokenizer.encode(text + "<ch>")
    ...
    def predict(self, evidence: List[str]) -> List[Tuple]:
        ...
        with torch.no_grad():
            ...
            probs = torch.log_softmax(logits[:,-1,:], dim=1).to(self.device)
        
        char_to_log_probs = {}

        for ch in self.single_char_tokens.keys():
            char_to_log_probs[ch] = []
            for i in self.single_char_tokens[ch]:
                char_to_log_probs[ch] += float(probs[0][i]),
        ...
        for i, ch in enumerate(self.single_char_tokens.keys()):
            if ch is ' ':
                ch = SPACE_CHAR
            else:
                ch = ch.upper()    
    ...
    def load(self) -> None:
        ...
        for ch in self.symbol_set_lower:
            ...
            index = self.tokenizer.encode(ch)[-1]
            self.single_char_tokens[ch] = [index]

            if ch.upper() != ch:
                index = self.tokenizer.encode(ch.upper())[-1]
                self.single_char_tokens[ch] += [index]
