...
class TestCausalLanguageModelAdapter(unittest.TestCase):
    ...
    def test_gpt2_init(self):
        ...
        self.assertTrue(isinstance(self.gpt2_model, CharacterLanguageModel))
    ...
    def test_opt_init(self):
        ...
        self.assertTrue(isinstance(self.opt_model, CharacterLanguageModel))
    ...
    def test_invalid_symbol_set(self):
        ...
        with self.assertRaises(InvalidSymbolSetException):
            lm = CausalLanguageModelAdapter(lang_model_name="gpt2")
    ...
    def test_invalid_model_name(self):
        ...
        with self.assertRaises(InvalidLanguageModelException):
            lm = CausalLanguageModelAdapter(lang_model_name="phonymodel")
            lm.set_symbol_set(DEFAULT_SYMBOL_SET)
    ...
    def test_non_mutable_evidence(self):
        ...
        self.gpt2_model.predict_character(evidence)
        self.assertEqual(evidence, evidence2)

        self.opt_model.predict_character(evidence)
        self.assertEqual(evidence, evidence2)

    def test_gpt2_identical(self):
        """Ensure predictions are the same for subsequent queries with the same evidence."""
        query1 = self.gpt2_model.predict_character(list("evidenc"))
        query2 = self.gpt2_model.predict_character(list("evidenc"))
    ...
    def test_opt_identical(self):
        """Ensure predictions are the same for subsequent queries with the same evidence."""
        query1 = self.opt_model.predict_character(list("evidenc"))
        query2 = self.opt_model.predict_character(list("evidenc"))
        for ((sym1, prob1), (sym2, prob2)) in zip(query1, query2):
            self.assertAlmostEqual(prob1, prob2, places=5)
            self.assertEqual(sym1, sym2)
    ...
    def test_gpt2_upper_lower_case(self):
        ...
        lc = self.gpt2_model.predict_character(list("EVIDENC"))
        uc = self.gpt2_model.predict_character(list("evidenc"))
    ...
    def test_opt_upper_lower_case(self):
        """Ensure predictions are the same for upper or lower case evidence."""
        lc = self.opt_model.predict_character(list("EVIDENC"))
        uc = self.opt_model.predict_character(list("evidenc"))
        for ((l_sym, l_prob), (u_sym, u_prob)) in zip(lc, uc):
            self.assertAlmostEqual(l_prob, u_prob, places=5)
            self.assertEqual(l_sym, u_sym)
    ...
    def test_gpt2_predict_start_of_word(self):
        ...
        symbol_probs = self.gpt2_model.predict_character(evidence=[])
    ...
    def test_opt_predict_start_of_word(self):
        ...
        symbol_probs = self.opt_model.predict_character(evidence=[])
        probs = [prob for sym, prob in symbol_probs]
    ...
    def test_gpt2_predict_middle_of_word(self):
        ...
        symbol_probs = self.gpt2_model.predict_character(evidence=list("TH"))
        probs = [prob for sym, prob in symbol_probs]
    ...
    def test_opt_predict_middle_of_word(self):
        ...
        symbol_probs = self.opt_model.predict_character(evidence=list("TH"))
        probs = [prob for sym, prob in symbol_probs]
    ...
    def test_gpt2_multiple_spaces(self):
        ...
        symbol_probs_before = self.gpt2_model.predict_character(list("the"))
        symbol_probs_after = self.gpt2_model.predict_character(list("the_"))
    ...
    def test_opt_multiple_spaces(self):
        """Test that the probability of space after a space is smaller than before the space"""
        symbol_probs_before = self.opt_model.predict_character(list("the"))
        symbol_probs_after = self.opt_model.predict_character(list("the_"))
        space_prob_before = (dict(symbol_probs_before))[SPACE_CHAR]
        space_prob_after = (dict(symbol_probs_after))[SPACE_CHAR]
        self.assertTrue(space_prob_before > space_prob_after)
