...
class ArtifactDetection:
    ...
    def __init__(
            ...
            eye_channels: Optional[List[str]]=None) -> None:
        ...
        self.log = None
        self.mne_data = self.raw_data_to_mne(raw_data, volts=False)
    ...
    def label_artifacts(
        ...
        # plot the data with annotations. This allows the user to reject additional bad epochs
        # correct annotations using MNE.
        if semi_automatic:
    ...
    def label_eog_events(
        ...
        postblink: float=.5,
        label: str='BAD_blink',
        threshold: float=80e-6) -> Optional[Tuple[mne.Annotations, list]]:
        """Label EOG artifacts.
...
def artifact_analysis(
    ...
    mne_data = semi_automatic_artifact_analysis(mne_data, percent_bad=percent_bad, verify_plot=semi_automatic_ar, extra_labels=session_annotations)
...
def semi_automatic_artifact_analysis(
        mne_data: mne.io.RawArray,
        percent_bad: float=50.0,
