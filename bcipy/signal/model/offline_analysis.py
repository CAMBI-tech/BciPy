...
from bcipy.helpers.acquisition import analysis_channels, analysis_channel_names_by_pos
...
def offline_analysis(
    ...
    Args:
        ...
        return_fig_handles(bool): whether or not to return handles for all figures generated
    ...
    raw_dat, _, channels, type_amp, fs = read_data_csv(data_folder + "/" + raw_data_file)
    ...
    # bandpass filter
    filtered_data = bandpass.butter_bandpass_filter(notch_filter_data, lp_filter, hp_filter, fs, order=filter_order)

    # downsample
    data = downsample.downsample(filtered_data, factor=downsample_rate)

    # Process triggers.txt
    _, t_t_i, t_i, offset = trigger_decoder(mode=mode, trigger_path=f"{data_folder}/{triggers_file}")
