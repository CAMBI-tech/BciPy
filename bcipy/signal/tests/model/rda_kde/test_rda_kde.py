...
class TestRdaKdeModelInternals(ModelSetup):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
    ...
    def setUp(self):
        np.random.seed(0)
        self.model = RdaKdeModel(k_folds=10)
    ...
    def test_kde_values(self):
        pca = MockPCA(n_components=0.9, num_ch=self.num_channel)
