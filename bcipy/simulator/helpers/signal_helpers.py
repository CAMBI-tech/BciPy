...
class EegRawDataProcessor(RawDataProcessor):
    ...
    def process(self, data_folder: str,
        ...
        if trial_window is None:
            ...
            trial_window = tuple(
                [0, trial_length])  # trial_length=0.5 -> trial_window=(0,0.5)
        ...
        corrected_trigger_timing = [
            timing + trial_window[0] for timing in trigger_timing
        ]
        ...
        channels_used = [
            channels[i] for i, keep in enumerate(channel_map) if keep == 1
        ]
