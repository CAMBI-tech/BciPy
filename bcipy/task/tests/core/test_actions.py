...
class TestActions(unittest.TestCase):
    ...
    def test_offline_analysis_action(self) -> None:
        when(actions).offline_analysis(parameters=self.parameters, alert_finished=any()).thenReturn(None)
        ...
        response = action.execute()
        self.assertIsInstance(response, TaskData)
        verify(actions, times=1).offline_analysis(parameters=self.parameters, alert_finished=any())
