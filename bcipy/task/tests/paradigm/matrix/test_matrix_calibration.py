...
class TestMatrixCalibration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        when(self.display).do_inquiry().thenReturn(self.mock_do_inquiry_response)
        when(self.display).wait_screen().thenReturn(None)

        when(bcipy.task.paradigm.matrix.calibration).init_calibration_display_task(
    ...
    def test_execute_save_stimuli_positions(self):
        ...
        verify(bcipy.task.paradigm.matrix.calibration, times=1).save_stimuli_position_info(
            any(), any(), task.screen_info)
        verify(self.display, times=self.parameters['stim_number']).do_inquiry()
        verify(task, times=self.parameters['stim_number']).write_trigger_data(any(), any())
        verify(task, times=1).write_offset_trigger()
    ...
    def test_write_trigger_data_first_run(self):
        ...
        when(bcipy.task.paradigm.matrix.calibration).offset_label('EEG').thenReturn('starting_offset')
        when(bcipy.task.paradigm.matrix.calibration).convert_timing_triggers(
        ...
        verify(bcipy.task.paradigm.matrix.calibration, times=1).offset_label(
            'EEG')
        verify(bcipy.task.paradigm.matrix.calibration, times=1).convert_timing_triggers(
