...
class TestDecisionMaker(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.decision_maker = DecisionMaker(1,3,state='',
                                            alphabet=list(string.ascii_uppercase) + ['<'] + [SPACE_CHAR],
                                            is_txt_sti=True,stimuli_timing=[1,.2],seq_constants=None)
    ...
    def test_decide_without_commit(self):
        """Test decide method with case of no commit using a fake probability distribution"""
    ...
    def test_update_with_letter(self):
        ...
        self.assertEqual(self.decision_maker.displayed_state,old_displayed_state+'E')
    ...
    def test_update_with_backspace(self):
        ...
        self.decision_maker.update(state=new_state)
        self.assertEqual(self.decision_maker.state,old_state+new_state)
        """Check that displayed state does not take into account the backspace """
        self.assertEqual(self.decision_maker.displayed_state,old_displayed_state[0:-1])
        self.assertLess(len(self.decision_maker.displayed_state),len(self.decision_maker.state))
