...
def _write_triggers_from_sequence_copy_phrase(array, file, copy_text, typed_text):
	...
	# get relevant spelling information to determine what was and should be typed
	spelling_length = len(typed_text)
	last_typed = typed_text[-1]
	correct_letter = copy_text[spelling_length - 1]

	# because there is the possiblility of incorrect letter and correction, we check here what is appropriate as a correct response
	...
	for i in array:
		...
		# determine what the triggers are: this assumes there is no target letter presentation.
		if x == 0:
			targetness = 'fixation'
