...
if __name__ == "__main__":
    ...
    cf_mcc_cols = [col for col in df.columns if 'CF' in col and 'mcc' in col and 'std' not in col and 'RF' not in col]
    ...
    breakpoint()
    of_ba_cols = [col for col in df.columns if 'OF' in col and 'balanced_accuracy' in col and 'std' not in col and 'RF' not in col]
    of_ba_df = df[of_ba_cols]
    cf_ba_cols = [col for col in df.columns if 'CF' in col and 'balanced_accuracy' in col and 'std' not in col and 'RF' not in col]
    ...
    for model in models:
        ...
        # result = permutation_ttest_mne([df[col] for col in cols])
        permutations = 50000
        t_obs, clust, p_value, h_s = permutation_cluster_1samp_test(
            diff, n_permutations=permutations, threshold=t_thresh, adjacency=None, n_jobs=None, tail=0)
    ...
    of_all_mcc_df = of_mcc_df.mean(axis=1).mean()
    cf_all_mcc_df = cf_mcc_df.mean(axis=1).mean()
    of_all_ba_df = of_ba_df.mean(axis=1).mean()
    cf_all_ba_df = cf_ba_df.mean(axis=1).mean()
